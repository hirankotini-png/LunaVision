  ...
  useEffect(() => {
    if (analysisResult) {
      try {
        ...
        // Prevent duplicate saving of the same session ID if it's the last one
        if (history.length === 0 || history[history.length - 1].result?.session_id !== analysisResult.session_id) {
          history.push({
            id: history.length + 1,
          ...
          localStorage.setItem('luna_vision_history', JSON.stringify(history));
        }
      } catch (e) {
